...
def load_data(in_file, max_example=None, relabeling=True, question_belong=[]):
    ...
    for inf in files:
        ...
        for i, q in enumerate(obj["questions"]):
            ...
            cur_answer = ord(obj["answers"][i]) - ord('A')
            cur_answer_label = [0, 0, 0, 0]
            cur_answer_label[cur_answer] = 1
            answers_label += cur_answer_label
            num_examples += 1
    ...
    return (documents, questions, options, answers, answers_label)
...
def vectorize(examples, word_dict,
              sort_by_len=True, verbose=True, concat=False, config=None):
    """
        Vectorize `examples`.
    ...
    in_y = []
    ans_label = []
    def pad_zero(st, maxlen):
        if len(st) < maxlen:
            return st + [0]*(maxlen - len(st))
        else:
            return st[:maxlen]
    ...
    for idx, (d, q, a) in enumerate(zip(examples[0], examples[1], examples[3])):
        ...
        if (len(seq1) > 0) and (len(seq2) > 0):
            ...
            for i in range(4):
                ...
                if len(option) > maxolen:
                    maxolen = len(option)
                assert len(option) > 0
                option_seq += [option]
                ans_label += [examples[4][i + idx * 4]]
            in_x3 += [option_seq]
            in_y.append(a)
    ...
    if sort_by_len:
        ...
        ans_label = [ans_label[i * 4 + k] for i in sorted_index for k in range(4)]
    new_in_x3 = []
    for i in in_x3:
        #print i
        new_in_x3 += i
    ...
    if config:
        for i in range(len(in_x1)):
            in_x1[i] = pad_zero(in_x1[i], config.para_limit)
        for i in range(len(in_x2)):
            in_x2[i] = pad_zero(in_x2[i], config.ques_limit)
        for i in range(len(new_in_x3)):
            new_in_x3[i] = pad_zero(new_in_x3[i], config.opt_limit)
    else:
        for i in range(len(in_x1)):
            in_x1[i] = pad_zero(in_x1[i], maxdlen)
        for i in range(len(in_x2)):
            in_x2[i] = pad_zero(in_x2[i], maxqlen)
        for i in range(len(new_in_x3)):
            new_in_x3[i] = pad_zero(new_in_x3[i], maxolen)
    return in_x1, in_x2, new_in_x3, in_y, ans_label
